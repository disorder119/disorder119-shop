(function () {
  ...
  async function finishGame(score,details,durationMs){
    ...
    if(run&&run.server){try{var response=await api("/games/score",{method:"POST",body:JSON.stringify({game:GAME_ID,runId:run.id,runToken:run.token,username:state.username,score:safeScore,durationMs:Math.round(durationMs),detail:Object.assign({version:SUBMISSION_VERSION,mode:PUBLIC_VERSION},details||{})})});qualified=Boolean(response.qualified);couponCode=String(response.couponCode||"");leaderboard=Array.isArray(response.leaderboard)?response.leaderboard:null;server=true;}catch(_){} }
  ...
  function startRunway(items){
    ...
    function spawn(progress){var target=requiredSlot(),roll=Math.random();if(roll<.11){objects.push({kind:"hazard",label:Math.random()<.55?"SOLD":"NO FIT",nx:.12+Math.random()*.76,z:1,speed:.24+progress*.12,drift:(Math.random()-.5)*.08,resolved:false});return;}var slot=roll<.72?target:META.slots[Math.floor(Math.random()*META.slots.length)],pool=pools[slot].length?pools[slot]:items,item=randomFrom(pool);if(!item)return;objects.push({kind:"piece",item:item,slot:slot,rare:isArchive(item)&&Math.random()<.34,img:imageFor(item),nx:.10+Math.random()*.80,z:1,speed:.205+progress*.13+Math.random()*.035,drift:(Math.random()-.5)*(.07+progress*.06),resolved:false,angle:(Math.random()-.5)*.12});}
    ...
    function overlap(p,o){var rx=Math.max(18,p.w*.28),ry=Math.max(20,p.h*.30);return Math.abs(player.x-p.x)<rx+player.r&&Math.abs(player.y-p.y)<ry+player.r;}
    ...
    function collectPiece(o,p,t){if(o.resolved)return;o.resolved=true;var wanted=requiredSlot();if(o.kind==="hazard"||o.slot!==wanted){wrong++;combo=1;score=Math.max(0,score-(o.kind==="hazard"?1900:1400));player.invUntil=t+460;addPop(o.kind==="hazard"?"SOLD −1.900":"WRONG PIECE −1.400",player.x,player.y-28,true);return;}currentLook.push(o.item);combo=Math.min(4,combo+.25);var price=Math.max(0,Number(o.item&&o.item.price)||0),gain=Math.round((1900+Math.min(2100,price*3))*combo);if(o.rare){gain+=3600;archiveCount++;slowUntil=t+1350;setFlash("ARCHIVE PIECE // +3.600",1050);}score=Math.min(149900,score+gain);addPop("+"+gain.toLocaleString("de-DE"),p.x,p.y-20,o.rare);updateDock();if(currentLook.length===META.slots.length){looks++;var lookBonus=9500+Math.round(combo*900);score=Math.min(149900,score+lookBonus);lastCompletedLook=currentLook.slice();setFlash("LOOK 0"+looks+" COMPLETE // +"+lookBonus.toLocaleString("de-DE"),1200);currentLook=[];combo=Math.min(4,combo+.4);window.setTimeout(function(){if(!stopped)updateDock();},480);}}
