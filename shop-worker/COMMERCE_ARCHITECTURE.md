...
State-changing public commerce endpoints require an `Idempotency-Key` (16-128 safe characters). Provider calls also receive provider-side idempotency keys where supported. D1 stores idempotency results for replay-safe retries.

PayPal webhook processing requires provider signature verification, checks transmission freshness and records `payment_events` with a unique provider event ID. Duplicate events become no-ops.
...
3. Bind the resulting D1 database as `DB`.
4. Keep checkout/customer-account feature flags disabled until provider credentials and end-to-end sandbox tests pass.

The v2 migration is additive and does not drop the existing legacy `orders` or `rental_requests` tables.
