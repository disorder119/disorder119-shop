...
MIETEN_INTRO_HTML = {
    "de": (
        ...
        "<h3>Wie die Miete funktioniert</h3>"
        "<ul>"
...
def cta_html(it, shop_config, home, lang):
    ...
    # Versandkosten gehoeren vor den Kaufknopf, nicht nur in die AGB: § 312a BGB
    # verlangt, dass der Gesamtpreis inklusive Versand vor der Bestellung
    # erkennbar ist. Der Betrag kommt aus derselben Konfiguration wie Warenkorb
    # und Worker, damit die drei Stellen nicht auseinanderlaufen koennen.
    ship_cents = shop_config.get("shippingFlatCents") or 0
    ship_notes = {
        "de": " Versand innerhalb Deutschlands pauschal {betrag} pro Bestellung.",
        "en": " Flat-rate shipping within Germany: {betrag} per order.",
        "fr": " Livraison forfaitaire en Allemagne : {betrag} par commande.",
    }
    note_text = trust_notes.get(lang, trust_notes["de"])
    if ship_cents:
        note_text += ship_notes.get(lang, ship_notes["de"]).format(betrag=euro_text(ship_cents, lang))
...
def json_ld(it, lang):
    ...
    if not sold and it.get("price", 0) > 0:
        ...
        # Versandkosten laut AGB Abschnitt 4. Der Wert kommt aus derselben
        # Konfiguration wie Warenkorb und Worker - stuende hier eine zweite
        # Zahl, koennte das Angebot bei Google anders aussehen als die Kasse.
        ship_cents = get_shop_config().get("shippingFlatCents") or 0
        if ship_cents:
...
def euro_text(cents, lang="de"):
    """5,90 € bzw. 5.90 EUR - eine Quelle fuer jeden Preistext auf der Seite."""
...
def get_shop_config():
    ...
    shipping_raw = raw.get("shippingFlatCents", 0)
    if not isinstance(shipping_raw, int) or isinstance(shipping_raw, bool) or shipping_raw < 0:
        raise SystemExit(
...
def static_page_content_html(slug, lang, shop_config):
    ...
    if slug in LEGAL_SLUG_KEY:
        ...
        html = html.replace("{versand}", euro_text(shop_config.get("shippingFlatCents") or 0, lang))
